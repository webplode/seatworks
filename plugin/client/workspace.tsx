...
function WorkspaceSeatworks(props: PluginWorkspacePanelProps & { initialTab: Tab }) {
  ...
  return <WorkspaceBody key={workspace.root} {...props} workspace={workspace} project={project?.slug} machine={machine} />;
}

function WorkspaceBody({ theme, layout, navigation, workspace, project, machine, initialTab }: PluginWorkspacePanelProps & {
  workspace: { name: string; root: string }; project?: string; machine: ReturnType<typeof useSeatworks>; initialTab: Tab;
}) {
  ...
  return <ScrollView style={{ flex: 1, backgroundColor: c.surface0 }} contentContainerStyle={{ padding: layout.compact ? 16 : 20, gap: 18 }}>
    ...
    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>{([['models','Team & models'],['work','Start work'],['supervisor','Overall Supervisor']] as const).map(([id,label]) => <Button key={id} label={label} theme={theme} tone={tab === id ? "accent" : "plain"} onPress={() => setTab(id)} />)}</View>
    ...
    {tab === "supervisor" ? <View style={{ gap: 16 }}>
      ...
      <Text style={{ color: c.foregroundMuted, lineHeight: 21 }}>Role profiles for new agents in this project. Changes save automatically. Existing agents keep their current provider and model.</Text>
      <SettingsCard><SettingsSelect label="Role profile" value={role?.id ?? ""} options={roles.map((r) => ({ value: r.id, label: r.label }))} onValueChange={setRoleId} /></SettingsCard>
      ...
      <Text style={{ color: c.foregroundMuted }}>{local.saving ? "Saving…" : local.saved ? "Saved" : "Project settings override machine defaults."}</Text>
      ...
      : <SupervisionPanel theme={theme} compact catalog={catalog} machine={global.values} projects={data.projects.filter((p) => p.slug === project)} onAdd={() => setSetup(true)} onSettings={(slug) => setTab(slug === "machine" ? "supervisor" : "models")} onFlow={() => setTab("activity")} onAgent={navigation ? (agentId) => navigation.openAgent({ agentId }) : undefined} />}
