...
function WorkspaceBody({ theme, layout, navigation, workspace, project, machine, initialTab }: PluginWorkspacePanelProps & {
  ...
  return <ScrollView style={{ flex: 1, backgroundColor: c.surface0 }} contentContainerStyle={{ padding: layout.compact ? 16 : 20, gap: 18 }}>
    ...
    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>{([['models','Team & models'],['work','New work'],['supervisor','Overall Supervisor']] as const).map(([id,label]) => <Button key={id} label={label} theme={theme} tone={tab === id ? "accent" : "plain"} onPress={() => setTab(id)} />)}</View>
