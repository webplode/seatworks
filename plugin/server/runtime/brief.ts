  ...
  for (const scope of binding.projects.filter(p => p.grants.includes("observe"))) {
    ...
    try {
      ...
      for (const lane of lanes) {
        ...
        if (report.gate === false) result.items.push({ id: `${scope.id}:${lane.id}:gate`, project: scope.name, kind: "tests", agent: lane.lead ?? null, scope: scope.id, lane: lane.id, diff, title: "Tests failed, so this can't be merged yet", plain: `Nothing to approve yet. "${titleOf(lane)}" is finished, but its tests fail. Open the Lead to see why.`, detail });
      ...
      for (const task of Object.values(ledger.tasks).filter(t => lanes.some(l => l.id === t.lane) && !["merged","cut"].includes(t.status))) {
        ...
        else if (task.status === "done") result.items.push({ id: `${scope.id}:${task.id}`, project: scope.name, kind: "review", agent: ledger.lanes[task.lane]?.lead ?? null, title: "Being reviewed", plain: "Nothing needed from you. A task is done and its Lead is checking it.", detail: task.title.slice(0,600) });
      }
